...
token ABENDPROMPT AFTER AT CARRYFORWARD CONFIRMED DATE8 DAYS DEADLINE 
...
token DRAFT RUNCYCLE
rule
	schedules		: schedule
							| schedules schedule

	schedule		: linecomments SCHEDULE {@processing=:SCHEDULE;@scanner.switch(:DEFAULT)} streamstmt ':' linecomments {@processing=:JOB;@scanner.switch(:DEFAULT)} jobstmts ENDSTMT
							{
	...
	streamattr	: DESCRIPTION STRING
                { @schedule.description=val[1] }
              | DRAFT
                { @schedule.draft=true }
              | FREEDAYS WORD freedaysopts
								{ 
  ...
  schedattrs  : schedattr
              | schedattrs schedattr

  schedattr   : RUNCYCLE WORD
              | dates fdrule
